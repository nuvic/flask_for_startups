...
def test_register_service_reject_duplicates(client, db, user_details):
    ...
    response_duplicate = client.post("/api/register",
                            json={
                                "username": user_details.test_username,
                                "email": user_details.test_email})
    ...
    user_model_list = db.session.query(User).filter_by(email=user_details.test_email).all()
...
def test_login_fail(client, db):
    ...
    expected_display_error = "There is no user with the entered credentials. Please try again."
